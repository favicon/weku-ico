...
_http2.default.createServer(app).listen(80, function () {
    return console.log('weku(http) is listening on port 80!');
});
...
_https2.default.createServer({
    ...
    return console.log('weku(https) is listening on port 443!');
});
...
function getCurrentBlock() {
    _steem2.default.api.getState('trending2', function (err, response) {
        _steem2.default.api.getBlock(response.props.head_block_number, function (err, result) {
            ...
            var blockContent = JSON.stringify(result, null, 4).replace("initminer", "wiwiminer").replace('"transaction_merkle_root": "0000000000000000000000000000000000000000",', '');
